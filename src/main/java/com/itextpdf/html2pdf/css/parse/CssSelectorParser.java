...
import com.itextpdf.html2pdf.css.selector.item.*;
...
public final class CssSelectorParser {
	 ...
	 *
	 * @param selector
	 *            the selectors in the form of a {@link String}
			...
			case '~':
				...
				if (selectorItems.size() == 0) {
					throw new IllegalArgumentException(MessageFormatUtil
							.format("Invalid token detected in the start of the selector string: {0}", firstChar));
				}
				...
				if (lastItem instanceof CssSeparatorSelectorItem) {
					if (curItem.getSeparator() == ' ') {
						...
						throw new IllegalArgumentException(MessageFormatUtil.format(
								"Invalid selector description. Two consequent characters occurred: {0}, {1}",
	 ...
	 * {@link CssSelectorParserMatch} in process.
	 *
	 * @param selectorItems
	 *            list of items to which new selector will be added to
	 * @param pseudoSelector
	 *            the pseudo selector
	 * @param match
	 *            the corresponding {@link CssSelectorParserMatch} that will be
	 *            updated.
	 */
	private static void appendPseudoSelector(List<ICssSelectorItem> selectorItems, String pseudoSelector,
			CssSelectorParserMatch match) {
		 ...
		 * :first-line, :first-letter, :before and :after). This compatibility
		...
		if (pseudoSelector.startsWith("::")) {
			...
			if (pseudoClassSelectorItem != null) {
				...
				// throw new
				// IllegalArgumentException(MessageFormatUtil.format(LogMessageConstant.UNSUPPORTED_PSEUDO_CSS_SELECTOR,
				// pseudoSelector));
			}
